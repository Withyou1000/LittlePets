...
ROWS = [
    ...
    ("jumping", 4, 8, "anticipation, lift, peak, descent, settle with a fuller 8-frame arc"),
    ("failed", 5, 8, "sad, failed, or deflated reaction"),
    ("shade-look", 6, 6, "raise one hand to shade the eyes, then look left and right"),
    ("shy", 7, 6, "cover the face and shake the head in a shy, embarrassed reaction"),
    ("blow-kiss", 8, 6, "send a blowing kiss with a tiny attached heart effect"),
]
...
STATE_REQUIREMENTS = {
    ...
    "jumping": [
        "Show the jump through pose and vertical body position only across 8 readable frames: anticipation, lift, early airborne, peak, slight hang, descent, landing, settle.",
    ...
    "shade-look": [
        "Show the pet raising one hand or paw to shade the eyes, then looking left and right in a searching motion.",
        "The motion should read as calm scanning rather than panic: hand comes up, head turns one side, returns through center, turns the other side, then relaxes.",
        "Do not add binoculars, telescopes, speech bubbles, punctuation, floating icons, or detached effects.",
    ],
    "shy": [
        "Show a shy, embarrassed reaction: one or both hands or paws come up to cover the face while the head gently shakes side to side.",
        "Let the body language feel bashful and soft rather than distressed or crying.",
        "Do not add sweat drops, floating punctuation, blush clouds, detached hearts, or other loose effects.",
    ],
    "blow-kiss": [
        "Show the pet giving a blowing kiss: gather the pose, bring a hand or paw near the mouth, then send the kiss outward and return.",
        "A tiny heart effect is allowed only if it is small, opaque, hard-edged, pixel-style, and attached to or overlapping the pet silhouette during the kiss moment.",
        "Do not add multiple floating hearts, sparkles, text, punctuation, or detached decorative effects.",
    ],
}
